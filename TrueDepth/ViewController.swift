...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate {
    ...
    func update(withFaceAnchor anchor: ARFaceAnchor) {
        ...
        DispatchQueue.main.async { [self] in
            ...
            let smoothThresholdNumber: Int = 80
            ...
            if(!isBlink){
                self.eyePositionIndicatorView.transform = CGAffineTransform(translationX: smoothEyeLookAtPositionX - self.view.frame.width / 2 + 30, y: smoothEyeLookAtPositionY - self.view.frame.height / 2 - 30)
            }
    ...
    func detectEyeBlink(anchor: ARFaceAnchor) {
        ...
        if ((blinkLeft?.decimalValue ?? 0.0) + (blinkRight?.decimalValue ?? 0.0)) > 0.3 {
            isBlink = true
            //print("blinking")
        }
        else{
